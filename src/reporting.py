...
class AnalysisReporter:
    """
    A class for generating and printing beautiful terminal reports from analysis DataFrames.
    """
    ...
    def __init__(self, track_summary_df: pd.DataFrame):
        ...
        Args:
            track_summary_df (pd.DataFrame): The final aggregated DataFrame from
    ...
    def _populate_table_with_data(
        ...
        Args:
            table (Table): The `rich` Table to populate.
            data (pd.DataFrame): A DataFrame containing the top tracks to display.
        """
    ...
    def print_overall_top_10(self) -> None:
        ...
        """
        self.console.rule("[bold green]🏆 Overall Top 10 Tracks 🏆[/bold green]")

        top_10_overall = self.df.sort_values(
            "adjusted_enjoyment_score", ascending=False
        ).head(10)

        table = self._create_results_table("Based on Bayesian Adjusted Enjoyment Score")
    ...
    def print_top_10_by_year(self) -> None:
        ...
        """
        self.console.rule("[bold green]📅 Top 10 Tracks by Year of First Listen 📅[/bold green]")

        # Ensure 'first_listen' is a datetime object to extract the year
        df_with_year = self.df.copy()
        df_with_year["year"] = pd.to_datetime(df_with_year["first_listen"]).dt.year

        # Get unique years and sort them
        years = sorted(df_with_year["year"].unique(), reverse=True) # Show recent years first
